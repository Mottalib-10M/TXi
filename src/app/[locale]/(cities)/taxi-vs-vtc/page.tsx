...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "taxiVsVtc" });
  ...
  return {
    ...
    openGraph: {
      title: t("metaTitle"),
      description: t("metaDescription"),
      url: `https://www.taxineo.fr/${locale}/taxi-vs-vtc`,
      siteName: "TaxiNeo",
      type: "article",
...
export default async function TaxiVsVtcPage() {
  const t = await getTranslations("taxiVsVtc");
  ...
  const comparisonRows = Array.from({ length: 6 }, (_, i) => ({
    label: t(`compRow${i + 1}Label`),
    taxi: t(`compRow${i + 1}Taxi`),
    vtc: t(`compRow${i + 1}Vtc`),
  }));
          ...
          <div className="max-w-4xl mx-auto fade-up fade-up-delay-1">
            <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden">
              <div className="grid grid-cols-3 bg-neutral-950 text-white">
                ...
                <div className="p-4 text-sm font-medium text-center">{t("compColTaxi")}</div>
                <div className="p-4 text-sm font-medium text-center">{t("compColVtc")}</div>
              </div>
              {comparisonRows.map((row, i) => (
                <div
                  key={row.label}
                  className={`grid grid-cols-3 ${i < comparisonRows.length - 1 ? "border-b border-neutral-100" : ""}`}
                >
                  <div className="p-4 text-sm font-medium text-neutral-900">{row.label}</div>
                  <div className="p-4 text-sm text-green-700 font-light text-center bg-green-50/50">
                    <Icon icon="solar:check-circle-linear" className="inline mr-1 text-green-600" />
                    {row.taxi}
                  </div>
                  <div className="p-4 text-sm text-neutral-500 font-light text-center">{row.vtc}</div>
                </div>
              ))}
            </div>
          </div>
