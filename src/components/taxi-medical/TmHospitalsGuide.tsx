...
export async function TmHospitalsGuide({ content, cityName }: { content: TmGeneratedContent; cityName: string }) {
  const t = await getTranslations("tm");
  const paragraphs = content.hospitalsGuide.split("\n\n").filter(Boolean);
  return (
    <section className="py-20 md:py-28">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-16 fade-up">
          <p className="text-sm font-medium text-neutral-500 mb-2 uppercase tracking-wider">{t("hospitalsSubtitle")}</p>
          <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">{t("hospitalsTitle", { cityName })}</h2>
        </div>
