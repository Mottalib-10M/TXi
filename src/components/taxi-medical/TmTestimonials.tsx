...
import type { TmTestimonial } from "@/data/taxi-medical-content-templates";
import { getTranslations } from "next-intl/server";

export async function TmTestimonials({ testimonials, cityName }: { testimonials: TmTestimonial[]; cityName: string }) {
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {testimonials.map((t, i) => (
            <div key={i} className={`bg-white border border-neutral-200 rounded-2xl p-6 card-hover fade-up fade-up-delay-${(i % 3) + 1}`}>
              ...
              <p className="text-sm text-neutral-600 font-light leading-relaxed mb-6">&quot;{t.text}&quot;</p>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-neutral-200 flex items-center justify-center text-xs font-semibold text-neutral-600">{t.initials}</div>
                <div>
