...
export async function MadQuartiersDetail({ content, cityName }: { content: MadGeneratedContent; cityName: string }) {
  const t = await getTranslations("mad");
  const paragraphs = content.quartiersDetail.split("\n\n").filter(Boolean);
  return (
    <section className="py-20 md:py-28 bg-neutral-50 border-t border-b border-neutral-100">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-16 fade-up">
          <p className="text-sm font-medium text-neutral-500 mb-2 uppercase tracking-wider">{t("quartiersSubtitle")}</p>
          <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">{t("quartiersTitle", { cityName })}</h2>
        </div>
