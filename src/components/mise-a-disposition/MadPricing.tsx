import type { MadPricingTier } from "@/data/mad-cities";
import { getTranslations, getLocale } from "next-intl/server";

export async function MadPricing({ pricing, cityName }: { pricing: MadPricingTier[]; cityName: string }) {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-200 bg-neutral-50">
                  <th className="text-left px-6 py-4 font-medium text-neutral-600">{t("pricingDuration")}</th>
                  <th className="text-center px-4 py-4 font-medium text-neutral-600">{t("pricingBerline")}</th>
                  <th className="text-center px-4 py-4 font-medium text-neutral-600">{t("pricingSUV")}</th>
                  <th className="text-center px-4 py-4 font-medium text-neutral-600">{t("pricingVan")}</th>
                </tr>
              </thead>
              <tbody>
                {pricing.map((tier, i) => (
                  <tr key={tier.duration} className={i < pricing.length - 1 ? "border-b border-neutral-100" : ""}>
                    ...
                    <td className="text-center px-4 py-4 text-neutral-600 font-light">{tier.suv}</td>
